...
void Processor::PulseClock()
{
	...
	if (_remainingCycles == 0)
	{
		...
		else
		{
			...
			if (_previousOpCode != OpCode::PREFIX)
			{
				info = OpCodeInfo::OpCodes + _memoryBus->Read(pc++);
				...
				(*Instructions[info->GetHexCode()])(args);
				_remainingCycles = info->GetCycleLengthMin();

				_previousOpCode = info->GetOpCode();
			}
			else
			{
				info = OpCodeInfo::OpCodesCB + _memoryBus->Read(pc++);
				...
				(*InstructionsCB[info->GetHexCode()])(args);
				_remainingCycles = info->GetCycleLengthMin();
...
uint16_t Processor::GetOperand(OperandType operand)
{
	...
	case OperandType::DataUINT16:
		data = _memoryBus->Read(pc++);
		data |= (_memoryBus->Read(pc++) << 8);
		break;
	case OperandType::AddressUINT16:
		addr = _memoryBus->Read(pc++);
		addr |= (_memoryBus->Read(pc++) << 8);
...
{
	uint16_t value = _memoryBus->Read(pc++);
	value |= static_cast<uint16_t>(_memoryBus->Read(pc++)) << 8;
	return value;
}
